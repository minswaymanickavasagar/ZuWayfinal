...
public class SubBannerTitleAdapter extends RecyclerView.Adapter<SubBannerTitleAdapter.MailViewHolder> {
    ...
    public SubBannerTitleAdapter(Context mContext, List<SubTitleModel> SubTitleModelList) {
    ...
    public void onBindViewHolder(@NonNull final MailViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        Shader textShader = new LinearGradient(0, 0, 0, holder.tvMenuTitle.getTextSize(),
                new int[]{
                        Color.parseColor("#622774"), Color.parseColor("#c53364")
                }, null, Shader.TileMode.CLAMP);
        holder.tvMenuTitle.setTextColor(mContext.getResources().getColor(R.color.appcolordarkyellow));
        holder.tvMenuTitle.getPaint().setShader(textShader);

        ArrayList<String> gridtitlearray = new ArrayList<String>();
        ArrayList<String> gridimgarray = new ArrayList<String>();
        final List<Productmodel> sublist = new ArrayList<>();
        ...
        subTitleGritAdapter = new SubTitleGritAdapter(mContext, R.layout.layout_banner_home, sublist);
        ...
        holder.hometitlegirt.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                cid = sublist.get(position).getCate_id();
                scid = sublist.get(position).getSubcate_id();
                String pid = sublist.get(position).getPid();
